> Source: https://www.necroraw.com.ru/docs/the-rules/the-pre-battle-sequence

# The Pre-Battle Sequence

[RAW ✔️](/docs/publication-index/rules-index) *Necromunda Core Rulebook (2023), p97*
...
defender benefits from the [Home Turf Advantage](/docs/battlefield-set-up/scenario-rules#home-turf-advantage), representing the gang fighting furiously to defend their
valuable turf from attackers. A gang fighting with the
[Home Turf Advantage](/docs/battlefield-set-up/scenario-rules#home-turf-advantage) is far less likely to bottle out and in some cases will fight ferociously until the last.
...
have special instructions for terrain. The default methods are detailed in full in [Battlefield Set-up](/docs/battlefield-set-up).
...
selection methods are explained in full in [Choosing Crews](/docs/battlefield-set-up/scenario-rules#choose-crew).
...
their gang is allowed to use (see [Gang Tactics](/docs/battlefield-set-up/scenario-rules#gang-tactics)).
...
standard deployment rules can be found in [Deployment](/docs/battlefield-set-up/scenario-rules#deployment).
