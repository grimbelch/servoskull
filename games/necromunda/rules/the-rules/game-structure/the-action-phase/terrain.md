> Source: https://www.necroraw.com.ru/docs/the-rules/game-structure/the-action-phase/terrain

# Terrain

[RAW ✔️](/docs/publication-index/rules-index) *Necromunda Core Rulebook (2023), p111*
...
immediately go [Out of Action](/docs/the-rules/game-structure/the-action-phase/resolve-hits#injury-dice), whilst a
vehicle will suffer an immediate [Catastrophic hit to its Drive](/docs/the-rules/game-structure/the-action-phase/resolve-hits-against-vehicles#drive-damage-table).
...
**Skimmers:** Skimmers ignore all difficult terrain and dangerous terrain when moving. However, if it ends its activation
not on the Battlefield Surface and on difficult terrain or
dangerous terrain, it must make a Handling test. If the test is passed, it has
successfully negotiated the terrain. If it is failed, it suffers an immediate Catastrophic hit to its Drive.

note

[RAW ✔️](/docs/publication-index/rules-index) *Book of the Outlands, p68*

**Walker:** For every 1" a walker moves through difficult terrain, it counts as having moved 2".
...
test again to avoid [falling](/docs/the-rules/game-structure/the-action-phase/terrain#falling).
If it would cause them to hit impassable terrain, move them the
shortest distance so as to be able to be placed. They only need to test once during
...
damage, see [Colliding with Terrain](/docs/the-rules/game-structure/the-action-phase/vehicle-impacts#colliding-with-terrain).
...
(see [Colliding with Terrain](/docs/the-rules/game-structure/the-action-phase/vehicle-impacts#colliding-with-terrain)).
...
## Zone Mortalis Stairs

[RAW ✔️](/docs/publication-index/rules-index) *Zone Mortalis Gang Tactics Cards*

If both players agree, the following rules for Stairs can be used:

**Stairs:** When climbing Stairs between the levels of a structure, there are
no modifiers to a fighter's movement. A fighter may end their action or
activation on Stairs as if they were a flat surface.

If a fighter goes from Standing to Prone while on Stairs, they are at risk
of falling down them. Should this happen, the fighter must make an Initiative
test. If the test is passed, nothing happens. If the test is failed, or if a
natural 1 is rolled, the fighter immediately falls down the Stairs. Place
the fighter at the bottom of the Stairs (or a mid-Stair landing if one is present).
If a fighter falls 3" or move down the Stairs, they suffer an S3, AP-, Damage 1 hit.

A fighter that has fallen in this way is immediately Prone and
Pinned and their activation ends.
...
will fall straight down by the shortest possible route to the next level down and will suffer a hit as described in [Resolving Hits: Falling](/docs/the-rules/game-structure/the-action-phase/resolve-hits#falling).

## Jumping Down

A fighter may attempt to jump down to a level below
or to Disembark from a vehicle. They must pass an
Initiative test with no modifier for the first 2" jumped,
but with a cumulative -1 modifier for every additional 2" jumped
(rounded up). If the test is failed, they fall and may
suffer a hit as described in [Resolving Hits: Falling](/docs/the-rules/game-structure/the-action-phase/resolve-hits#falling).
...
If a fighter falls or fails to Jump Down a distance of 3" or more, they will take a hit as described on in [Resolve Hits: Falling](/docs/the-rules/game-structure/the-action-phase/resolve-hits#falling) based on how far they fell, rounded up to
the nearest inch.
...
possible distance so that the two are not overlapping.
Once the hits have been resolved, and if neither fighter
...
If a falling fighter lands within ½" of a platform edge,
they must pass an Initiative test or will fall again.
...
pass a [Loss of Control test](/docs/the-rules/game-structure/the-action-phase/resolve-hits-against-vehicles#loss-of-control-tests) with a
cumulative -1 modifier for every 3" fallen, rounding
up to the nearest inch. In addition they take a hit as described in [Resolve Hits: Falling](/docs/the-rules/game-structure/the-action-phase/resolve-hits#falling), resolving damage against the
Rear Toughness and Drive

## Underhive Terrain

[![terrain-features.avif](/assets/images/terrain-features-865ec4bdf6822d0cfda43179bd31f7ea.avif)](/assets/files/terrain-features-865ec4bdf6822d0cfda43179bd31f7ea.avif)

**1. Ladders:** These can be used to climb to different
levels.

**2. Railings:** These help to reduce the chance of falling
off from a platform.

**3. Ductway:** These can be crawled through or
have weapons fired through them to the other side.

**4. Obstacle:** These provide cover to models behind
them by blocking line of sight.

**5. Doors:** These can be set to either open or closed,
and interacted with using the Operate Door (Simple)
action.

**6. Access Terminal:** These control the door they are
next to and can be used to open and close doors using
the Access Terminal (Basic) action.

**7. Steps:** These can be climbed by stepping up without
using any extra movement.
