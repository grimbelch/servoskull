...
def _normalize(text: str) -> str:
    ...
    text = text.replace("", "")  # failed glyph decodes

    # 1. Line-by-line filter for running headers, footers, standalone page numbers
    lines = text.split("\n")
    filtered = []
    for line in lines:
        s = line.strip()
        if re.match(r"^(Warhammer Fantasy Rolepl?\s*ay|WFRP CORE RULEBOOK|WARHAMMER 40,000|CLASS AND CAREERS|CHARACTER|SKILLS AND TALENTS|\d{1,3})$", s, re.I):
            continue
        if re.match(r"^[\.\*\s\-_]{2,}$", s):
            continue
        filtered.append(line)
    text = "\n".join(filtered)

    # 2. Fix fragmented bold & italic markdown tags (e.g. **C** **lass** **T** **rappings** -> **Class Trappings**)
    prev = None
    while prev != text:
        prev = text
        text = re.sub(r"\*\*([^\*\n]+)\*\*\s*\*\*([^\*\n]+)\*\*", r"**\1\2**", text)
    ...
    # Clean multiple consecutive asterisks like ****Class**** -> **Class**
    text = re.sub(r"\*{3,}", "**", text)

    # Fix space after punctuation before word inside bold, e.g. **11:Consumers** -> **11: Consumers**
    text = re.sub(r"(\d+[\:\)])([A-Za-z])", r"\1 \2", text)

    # 3. Fix initial letter kerning splits: C lass -> Class, T rappings -> Trappings, Rolepl ay -> Roleplay
    text = re.sub(r"\b([A-Z])\s+([a-z]{2,})\b", r"\1\2", text)
    text = re.sub(r"\b([A-Za-z]+)pl ay\b", r"\1play", text)
    text = re.sub(r"([a-zA-Z]+)\s*-\s*([a-zA-Z]+)", r"\1\2", text)

    # 4. Strip stray dot tokens like **..** or **...** or **.**
    text = re.sub(r"\s*\*\*\.[\.\s]*\*\*\s*", " ", text)
    text = re.sub(r"\s+\.[\.\s]*\b", " ", text)
    text = re.sub(r"\.{4,}", " ... ", text)

    # 5. Clean whitespace spacing
...
def _page_title(raw_text: str, fallback: str) -> str:
    ...
    raw_clean = _normalize(raw_text)
    for line in raw_clean.splitlines()[:6]:
        s = line.strip()
        if s.isdigit():
            continue
        if s.lower() in ("contents", "table of contents", "index"):
            return "Table of Contents"
        if len(re.sub(r"[^A-Za-z]", "", s)) >= 3:
            t = re.sub(r"\s+", " ", s)[:80].strip()
