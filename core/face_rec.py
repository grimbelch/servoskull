...
try:
    _cascade_path = cv2.data.haarcascades + "haarcascade_frontalface_default.xml" if hasattr(cv2, "data") else "haarcascade_frontalface_default.xml"
    _face_cascade = cv2.CascadeClassifier(_cascade_path) if hasattr(cv2, "CascadeClassifier") else None
except Exception:
    _face_cascade = None
