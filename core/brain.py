...
from core import tools_schema




_TOOLS = tools_schema.build_tools()
...
_SPOTIFY_RE = re.compile(
    r"\[SPOTIFY(?::([^\]|]+?)(?:\s*\|\s*on:\s*([^\]]+))?)?\]|\[SPOTIFY_(PAUSE|RESUME|SKIP)\]|\[SPOTIFY_TRANSFER:\s*([^\]]+)\]"
)
...
def _tool_bluetooth_disconnect(i):
    from core import bluetooth_ctrl, config
    identifier = str(i.get("identifier", "all")).strip()
    success = bluetooth_ctrl.disconnect(identifier)
    return (
        f"Disconnected from Bluetooth speaker. Default system audio output restored to {config.SKULL_NAME}."
        if success
...
def _tool_connect_bambu_printer(i):
    ...
    import pathlib
    import time
    from core import config, bambu_ctrl
...
if _brain_module and hasattr(_brain_module, 'get_handlers'):
    _TOOL_REGISTRY.update(_brain_module.get_handlers())
...
def respond(user_text: str, speaker_name: str | None = None, on_tool_use=None) -> tuple[str, list[tuple]]:
    ...
    def _extract_spotify(match):
        cmd = match.group(3)
        transfer_device = match.group(4)
        if cmd:
            cmds.append((cmd.lower(),))
        elif transfer_device:
            cmds.append(("transfer", transfer_device.strip()))
        else:
            query = match.group(1).strip() if match.group(1) else ""
            device = match.group(2).strip() if match.group(2) else None
            if query:
                cmds.append(("play", query, device))
        return ""
...
def record_assistant_turn(text: str) -> None:
    ...
    db.append_history("assistant", text)
    _load_history()
...
def _get_idle_prompt() -> str:
    return config.PERSONALITY.get("idle_news_prompt", (
        f"You are {config.SKULL_NAME}. Reinterpret ONE news item. "
